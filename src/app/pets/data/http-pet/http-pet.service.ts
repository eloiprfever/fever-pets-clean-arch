...
export class HttpPetService implements PetService {
  ...
  getPets(): Observable<PetModel[]> {
    return this.http.get<PetResponse[]>(this.API_URL).pipe(map((pets) => pets.map((pet) => this.petMapper.mapFrom(pet))));
  }

  getPetById(id: number): Observable<PetModel> {
    return this.http.get<PetResponse>(`${this.API_URL}/${id}`).pipe(map((pet) => this.petMapper.mapFrom(pet)));
  }

  createPet(pet: PetModel): Observable<PetModel> {
    return this.http.post<PetResponse>(`${this.API_URL}`, { ...this.petMapper.mapTo(pet) }).pipe(map((pet) => this.petMapper.mapFrom(pet)));
  }
}
